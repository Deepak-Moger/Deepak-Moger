...
<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&height=240&text=Deepak%20Moger&fontSize=58&fontAlign=50&fontAlignY=36&desc=Machine%20Learning%20Engineer%20%C2%B7%20Data%20Storyteller%20%C2%B7%20Systems%20Thinker&descAlign=50&descAlignY=60&descSize=16&color=0:0F2027,50:203A43,100:2C5364" alt="Header banner — Deepak Moger, Machine Learning Engineer"/>
</p>
...
<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&pause=1000&color=00C2FF&center=true&vCenter=true&width=820&lines=Interrogating+data+until+it+confesses.;Building+ML+systems+that+ship+real+value.;From+raw+signals+to+production+intelligence.;Treating+suspiciously+perfect+metrics+as+a+red+flag.;Hunting+drift+before+it+becomes+an+incident." alt="Animated tagline rotation"/>
</p>

<p align="center">
  <b>Machine Learning Engineer</b> &nbsp;·&nbsp; <b>Data Storyteller</b> &nbsp;·&nbsp; <b>Problem Solver</b>
  <br/>
  <sub>Turning noisy data into decisions, predictions, and production-ready intelligence.</sub>
</p>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=Deepak-moger&label=Profile%20Views&color=0e75b6&style=flat" alt="Profile views counter"/>
  <img src="https://img.shields.io/github/followers/Deepak-moger?label=Followers&style=flat&color=1F6FEB" alt="GitHub followers"/>
  <img src="https://img.shields.io/github/stars/Deepak-moger?affiliations=OWNER&label=Total%20Stars&style=flat&color=F39C12" alt="Total repository stars"/>
  <img src="https://img.shields.io/badge/Open%20to-Collaboration-10B981?style=flat" alt="Open to collaboration"/>
  <img src="https://img.shields.io/badge/Status-Shipping-22C55E?style=flat" alt="Currently shipping"/>
</p>

<p align="center">
  <img src="https://img.shields.io/badge/Focus-Machine%20Learning-0A66C2?style=for-the-badge" alt="Focus: Machine Learning"/>
  <img src="https://img.shields.io/badge/Role-Data%20Storyteller-1F6FEB?style=for-the-badge" alt="Role: Data Storyteller"/>
  <img src="https://img.shields.io/badge/Mindset-Research%20Driven-0E8A16?style=for-the-badge" alt="Mindset: Research Driven"/>
</p>
...
```mermaid
flowchart LR
    A["Suspiciously<br/>Perfect Metric"] --> B{Investigate}
    B -->|"data leak?"| C[Audit Splits]
    B -->|"target leakage?"| D[Trace Feature Origins]
    B -->|"trivial baseline?"| E[Compare vs Constant Predictor]
    B -->|"overfit?"| F[Cross-Validate Aggressively]
    C --> G[Honest Metric]
    D --> G
    E --> G
    F --> G
...
<a id="the-engineers-mental-model"></a>
...
```mermaid
flowchart TD
    Q["Vague Business Question<br/><i>'Can we predict X?'</i>"] --> R[Reframe as Measurable Outcome]
    R --> M[Define the Metric That Matters]
    M --> D{Do we have the<br/>right data?}
    D -->|No| DC[Data Collection / Instrumentation]
    D -->|Yes| EDA[Exploratory Data Analysis]
    DC --> EDA
    EDA --> B[Establish Honest Baseline]
    B --> H[Hypothesize Model Family]
    H --> T[Train · Validate · Iterate]
    T --> C{Beats Baseline<br/>Convincingly?}
    C -->|No| H
    C -->|Yes| CAL[Calibrate Probabilities]
    CAL --> EXP[Explainability & Error Analysis]
    EXP --> SHIP[Ship to Production]
    SHIP --> MON[Monitor · Detect Drift · Retrain]
    MON -->|drift detected| T
    MON -->|stable| OBS[Ongoing Observation]
...
<a id="core-tech-stack"></a>

## Core Tech Stack

<p align="center">
  <img src="https://skillicons.dev/icons?i=python,tensorflow,pytorch,sklearn,opencv,mysql,mongodb,postgres,docker,git,github,vscode,linux&perline=7" alt="Core tech stack icons"/>
</p>

<p align="center">
  <img src="https://img.shields.io/badge/NLP-NVIDIA%20Riva-76B900?style=for-the-badge&logo=nvidia&logoColor=white" alt="NVIDIA Riva"/>
  <img src="https://img.shields.io/badge/ML-Random%20Forest%20%7C%20XGBoost-1D4ED8?style=for-the-badge" alt="Random Forest and XGBoost"/>
  <img src="https://img.shields.io/badge/Research-Drift%20Detection-0F766E?style=for-the-badge" alt="Drift Detection"/>
</p>
...
<a id="system-architecture-lens"></a>
...
flowchart LR
    ...
    subgraph Store["Storage and Features"]
        ST1[(Data Lake)]
        ST2[(Feature Store)]
        ST3[(Vector DB)]
    end

    subgraph Train["Training and Experimentation"]
        T1[Notebooks]
    ...
    classDef obs fill:#FEE2E2,stroke:#DC2626,color:#7F1D1D
...
<a id="what-im-building-right-now"></a>
...
flowchart LR
    ...
    B --> C[XGBoost Ensemble]
    C --> D[Raw Probabilities]
    D --> E[Calibration Layer]
    E --> F{Reliability<br/>Check}
    F -->|miscalibrated| E
    F -->|trustworthy| G[Match Forecast]
```
...
sequenceDiagram
    ...
    participant A as ASR (Riva)
    participant T as Translator
    participant V as TTS (Riva)
    participant L as Listener

    U->>A: speech (lang A)
    A->>T: transcript (lang A)
    T->>V: translated text (lang B)
    V->>L: speech (lang B)
    Note over U,L: target: sub-second end-to-end
```
...
<a id="the-ml-lifecycle-i-live-by"></a>
...
stateDiagram-v2
    [*] --> Framing
    Framing --> DataAudit: problem is well-defined
    DataAudit --> Framing: data does not support the question
    DataAudit --> Baseline: data is sufficient
    Baseline --> Modeling
    Modeling --> Evaluation
    Evaluation --> Modeling: not yet good enough
    Evaluation --> Calibration: predictive enough
    Calibration --> Explainability
    Explainability --> Staging
    Staging --> Production: passes shadow tests
    Staging --> Modeling: regressions detected
...
sequenceDiagram
    ...
    C->>I: telemetry snapshots
    I->>F: normalized signals
    F->>M: feature vectors
    M-->>F: anomaly score
    M->>E: anomalous slices
    E->>A: ranked finding + rationale
    A->>A: dedupe, prioritize, route
    Note over A: humans see signal, not noise
```
...
<p align="center">
  <picture>
    ...
    <img src="https://github-readme-stats.vercel.app/api?username=Deepak-moger&show_icons=true&theme=default&hide_border=true" alt="Aggregate GitHub statistics" height="165"/>
  </picture>
  <picture>
    ...
    <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Deepak-moger&layout=compact&theme=default&hide_border=true" alt="Most-used programming languages" height="165"/>
  </picture>
</p>
...
<p align="center">
  <picture>
    ...
    <img src="https://github-readme-streak-stats.herokuapp.com/?user=Deepak-moger&theme=default&hide_border=true" alt="Contribution streak"/>
  </picture>
</p>
...
<p align="center">
  <picture>
    ...
    <img src="https://github-readme-activity-graph.vercel.app/graph?username=Deepak-moger&theme=github-light&hide_border=true&area=true" alt="Contribution activity over time"/>
  </picture>
</p>

---

<a id="pinned-projects"></a>

## Pinned Projects

<p align="center">
  <a href="https://github.com/Deepak-moger/ArcSense">
    <img src="https://github-readme-stats.vercel.app/api/pin/?username=Deepak-moger&repo=ArcSense&theme=tokyonight&hide_border=true" alt="ArcSense repository card"/>
  </a>
  <a href="https://github.com/Deepak-moger/Dream11Oracle">
    <img src="https://github-readme-stats.vercel.app/api/pin/?username=Deepak-moger&repo=Dream11Oracle&theme=tokyonight&hide_border=true" alt="Dream11Oracle repository card"/>
  </a>
</p>

<p align="center">
  <a href="https://github.com/Deepak-moger/IPLOracle">
    <img src="https://github-readme-stats.vercel.app/api/pin/?username=Deepak-moger&repo=IPLOracle&theme=tokyonight&hide_border=true" alt="IPLOracle repository card"/>
  </a>
  <a href="https://github.com/Deepak-moger/AIHEALTH-NAVIGATOR">
    <img src="https://github-readme-stats.vercel.app/api/pin/?username=Deepak-moger&repo=AIHEALTH-NAVIGATOR&theme=tokyonight&hide_border=true" alt="AIHEALTH-NAVIGATOR repository card"/>
  </a>
</p>
...
<table>
  <tr>
    <td width="50%" valign="top">
      <h3>ArcSense</h3>
      <p><b>The Idea:</b> A TypeScript system for orchestrating structured intelligence workflows — pipelines where each stage is typed, traceable, and composable.</p>
      <p><b>Why It Is Interesting:</b> Most intelligence pipelines are loose chains of scripts. ArcSense argues for <i>structured</i>, type-safe orchestration where failures are caught at compile time, not in production.</p>
      <p><b>Stack:</b> TypeScript</p>
      <p><a href="https://github.com/Deepak-Moger/ArcSense">View Repository &rarr;</a></p>
    </td>
    <td width="50%" valign="top">
      <h3>Dream11Oracle</h3>
      <p><b>The Idea:</b> A fantasy sports prediction engine designed for <i>decision-ready</i> ML outputs — calibrated probabilities, not vibes.</p>
      <p><b>Why It Is Interesting:</b> Fantasy sports are a near-perfect testbed for calibration: small probability differences compound into very real outcomes, so the metric you optimize must match the decision being made.</p>
      <p><b>Stack:</b> Python</p>
      <p><a href="https://github.com/Deepak-Moger/Dream11Oracle">View Repository &rarr;</a></p>
    </td>
  </tr>
  <tr>
    <td width="50%" valign="top">
      <h3>IPLOracle</h3>
      <p><b>The Idea:</b> Cricket outcome forecasting using historical signals, trend modeling, and conditional context — venue, weather proxies, team form, and matchup dynamics.</p>
      <p><b>Why It Is Interesting:</b> Cricket is rich in <i>structured chaos</i> — perfect for engineered features and ensemble models that respect the game's conditional nature.</p>
      <p><b>Stack:</b> Jupyter Notebook · Python</p>
      <p><a href="https://github.com/Deepak-Moger/IPLOracle">View Repository &rarr;</a></p>
    </td>
    <td width="50%" valign="top">
      <h3>AIHEALTH-NAVIGATOR</h3>
      <p><b>The Idea:</b> An AI healthcare assistant concept for guided, conversational symptom triage and information support.</p>
      <p><b>Why It Is Interesting:</b> Healthcare AI lives or dies on <i>safety scaffolding</i> — knowing when not to answer is as important as knowing when to. This project is an exploration of those guardrails.</p>
      <p><b>Stack:</b> Python</p>
      <p><a href="https://github.com/Deepak-Moger/AIHEALTH-NAVIGATOR">View Repository &rarr;</a></p>
    </td>
  </tr>
  <tr>
    <td width="50%" valign="top">
      <h3>agent-automotive</h3>
      <p><b>The Idea:</b> Real-time AI agents that reduce breakdown risk and improve service planning by reasoning over vehicle telemetry.</p>
      <p><b>Why It Is Interesting:</b> Predictive maintenance is one of the highest-ROI applications of ML, and agentic patterns offer a path to <i>contextual</i> recommendations rather than static thresholds.</p>
      <p><b>Stack:</b> Python</p>
      <p><a href="https://github.com/Deepak-Moger/agent-automotive">View Repository &rarr;</a></p>
    </td>
    <td width="50%" valign="top">
      <h3>ai-agent-challenge</h3>
      <p><b>The Idea:</b> An agentic parser pipeline that extracts structured data from bank statement PDFs — varied layouts, inconsistent formats, real-world mess.</p>
      <p><b>Why It Is Interesting:</b> PDFs are the natural enemy of structured data. Solving this elegantly with an agentic approach is a study in <i>graceful degradation</i> and recovery.</p>
      <p><b>Stack:</b> Python</p>
      <p><a href="https://github.com/Deepak-Moger/ai-agent-challenge">View Repository &rarr;</a></p>
    </td>
  </tr>
</table>
...
```mermaid
quadrantChart
    title Project Landscape — Research Depth vs Production Readiness
    x-axis "Exploratory" --> "Production-Hardened"
    y-axis "Narrow Scope" --> "Systems-Level"
    quadrant-1 "Flagship Systems"
    quadrant-2 "Research Frontiers"
    quadrant-3 "Sandbox Experiments"
    quadrant-4 "Shipped Utilities"
    "Drift Hunter": [0.75, 0.85]
    "ArcSense": [0.65, 0.78]
    "Dream11Oracle": [0.7, 0.55]
    "IPLOracle": [0.6, 0.5]
    "AIHEALTH-NAVIGATOR": [0.35, 0.7]
    "agent-automotive": [0.55, 0.72]
    "ai-agent-challenge": [0.5, 0.45]
    "Voice-to-Voice MT": [0.4, 0.8]
```
...
```mermaid
flowchart LR
    P[Prototype<br/>in Notebook] --> R[Refactor<br/>into Modules]
    R --> TST[Add Tests<br/>Code + Data]
    TST --> C[Containerize]
    C --> CI[CI/CD Pipeline]
    CI --> D[Deploy]
    D --> M[Monitor]
...
<p align="left">
  <a href="https://github.com/Deepak-moger">
    <img src="https://img.shields.io/badge/GitHub-Deepak--moger-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub profile"/>
  </a>
  <a href="https://www.linkedin.com/in/deepak-narayan-moger/">
    <img src="https://img.shields.io/badge/LinkedIn-Deepak%20Narayan%20Moger-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn profile"/>
  </a>
</p>
...
<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&section=footer&height=140&color=0:0F2027,50:203A43,100:2C5364&text=Let%27s%20build%20something%20honest.&fontSize=22&fontColor=ffffff&fontAlignY=70" alt="Footer banner — Let's build something honest"/>
</p>

<p align="center">
  <a href="#top">
    <img src="https://img.shields.io/badge/%E2%86%91-Back%20to%20top-0A66C2?style=flat-square" alt="Back to top"/>
  </a>
</p>
